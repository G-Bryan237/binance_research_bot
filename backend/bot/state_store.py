...
class BotStateStore:
        ...
        with self._connect() as conn:
            conn.executescript(
                ...
                CREATE TABLE IF NOT EXISTS recent_trades (
                    ...
                    simulated_friction REAL NOT NULL DEFAULT 0,
                    execution_model TEXT NOT NULL DEFAULT 'paper',
                ...
                CREATE TABLE IF NOT EXISTS signal_journal (
                    ...
                    r_multiple REAL,
                    close_reason TEXT,
                    forecast_status TEXT NOT NULL DEFAULT 'WATCHING',
                    forecast_result TEXT NOT NULL DEFAULT 'watching',
                    forecast_reason TEXT NOT NULL DEFAULT '',
                    forecast_expires_at_utc TEXT,
                    forecast_hit_at_utc TEXT,
                    forecast_hit_price REAL,
                    forecast_r_multiple REAL,
                    forecast_updated_at_utc TEXT,
                    profile_id TEXT NOT NULL DEFAULT 'default'
                );
    ...
    def _run_migrations(self) -> None:
        ...
        self.migrate_trade_simulation_columns()
        self.migrate_signal_forecast_columns()
    ...
    def add_trade(self, trade_info: Dict[str, Any], profile_id: str = "default") -> None:
        row = {
            ...
            "fee_paid": float(trade_info.get("fee_paid") or 0.0),
            "funding_paid": float(trade_info.get("funding_paid") or 0.0),
            "simulated_friction": abs(float(trade_info.get("fee_paid") or 0.0)) + abs(float(trade_info.get("funding_paid") or 0.0)),
            "execution_model": str(trade_info.get("execution_model") or "paper-slippage-fee-model"),
        ...
        with self._connect() as conn:
            conn.execute(
                ...
                INSERT INTO recent_trades (
                    ...
                    strategy_module, regime, close_reason, partial_exit, fee_paid, funding_paid,
                    simulated_friction, execution_model, profile_id
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    row["timestamp"],
    ...
    def add_signal(self, signal_info: Dict[str, Any], profile_id: str = "default") -> int:
        ...
        row = {
            ...
            "forecast_status": str(signal_info.get("forecast_status") or "WATCHING"),
            "forecast_result": str(signal_info.get("forecast_result") or "watching"),
            "forecast_reason": str(signal_info.get("forecast_reason") or "Waiting for price to test TP/stop before expiry"),
            "forecast_expires_at_utc": signal_info.get("forecast_expires_at_utc"),
        ...
        with self._connect() as conn:
            conn.execute(
                ...
                INSERT OR IGNORE INTO signal_journal (
                    ...
                    status_reason, forecast_status, forecast_result, forecast_reason,
                    forecast_expires_at_utc, profile_id
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    row["signal_key"],
    ...
    def update_signal_forecast(
        ...
        **kwargs: Any,
        ...
        now = _now_iso()
        values: list[Any] = [forecast_status, forecast_result, forecast_reason, now, now]
        for key, value in kwargs.items():
            if key not in allowed:
                continue
            updates.append(f"{key} = ?")
        ...
        with self._connect() as conn:
            row = conn.execute(
                "SELECT forecast_status FROM signal_journal WHERE id = ?",
                (int(signal_id),),
            ).fetchone()
            if row is None or str(row["forecast_status"] or "WATCHING") != "WATCHING":
                return
            conn.execute(
                f"UPDATE signal_journal SET {', '.join(updates)} WHERE id = ?",
                values,
            ...
            if updated is not None:
                self._insert_event(conn, "signal_updated", self._signal_row_to_dict(updated))

    def get_unresolved_signal_forecasts(
        self,
        *,
        symbol: str,
        market: str,
        ...
        with self._connect() as conn:
            rows = conn.execute(
                ...
                WHERE symbol = ?
                  AND market = ?
                  AND profile_id = ?
                  AND forecast_status = 'WATCHING'
                ORDER BY id ASC
                LIMIT ?
                """,
                (symbol, market, profile_id, safe_limit),
            ).fetchall()
    ...
    def _signal_row_to_dict(self, row: sqlite3.Row) -> Dict[str, Any]:
        return {
            ...
            "forecast_status": str(row["forecast_status"]),
            "forecast_result": str(row["forecast_result"]),
            "forecast_reason": str(row["forecast_reason"]),
            "forecast_expires_at_utc": row["forecast_expires_at_utc"],
            "forecast_hit_at_utc": row["forecast_hit_at_utc"],
            "forecast_hit_price": None if row["forecast_hit_price"] is None else float(row["forecast_hit_price"]),
            "forecast_r_multiple": None if row["forecast_r_multiple"] is None else float(row["forecast_r_multiple"]),
            "forecast_updated_at_utc": row["forecast_updated_at_utc"],
    ...
    def get_signal_stats(self) -> Dict[str, Any]:
        with self._connect() as conn:
            rows = conn.execute(
                """
                SELECT strategy_module, status, forecast_result, quality_label, quality_score
                FROM signal_journal
                """
            ).fetchall()
        ...
        forecast_expired_results = {"expired_no_hit"}
        ...
        for row in rows:
            ...
            forecast_result = str(row["forecast_result"] or "watching")
            quality_label = str(row["quality_label"] or "medium")
            quality_score = float(row["quality_score"] or 0.0)
            buckets = [totals, ensure_bucket(by_module, module), ensure_bucket(by_quality, quality_label)]

            for bucket in buckets:
                bucket["total_signals"] += 1
                bucket["avg_quality_score"] += quality_score
                ...
                if forecast_result in forecast_expired_results:
        ...
        def finalize(bucket: Dict[str, Any]) -> None:
            resolved_trades = bucket["succeeded_signals"] + bucket["failed_signals"]
            forecast_directional = bucket["forecast_successes"] + bucket["forecast_failures"]
            bucket["success_rate"] = (bucket["succeeded_signals"] / resolved_trades) if resolved_trades else 0.0
            ...
            bucket["forecast_resolution_rate"] = (bucket["forecast_resolved"] / bucket["total_signals"]) if bucket["total_signals"] else 0.0
            bucket["tp_hit_rate"] = (bucket["forecast_successes"] / bucket["forecast_resolved"]) if bucket["forecast_resolved"] else 0.0
            bucket["stop_hit_rate"] = (bucket["stop_hits"] / bucket["forecast_resolved"]) if bucket["forecast_resolved"] else 0.0
            bucket["expired_rate"] = (bucket["expired_no_hit"] / bucket["forecast_resolved"]) if bucket["forecast_resolved"] else 0.0
            ...
            out.append(
                {
                    ...
                    "simulated_friction": float(row["simulated_friction"] or 0.0),
                    "execution_model": str(row["execution_model"] or "paper-slippage-fee-model"),
                    "profile_id": str(row["profile_id"]) if row["profile_id"] else "default",
                }
            )
        return out
    ...
    def get_simulation_stats(self) -> Dict[str, Any]:
        with self._connect() as conn:
            trade_row = conn.execute(
                ...
                SELECT
                    ...
                    COALESCE(SUM(funding_paid), 0) AS total_funding,
                    COALESCE(SUM(simulated_friction), 0) AS total_friction,
                    COALESCE(SUM(pnl), 0) AS net_pnl,
                    COALESCE(SUM(pnl + simulated_friction), 0) AS gross_estimated_pnl,
                    COALESCE(SUM(CASE WHEN partial_exit = 1 THEN 1 ELSE 0 END), 0) AS partial_exits
                FROM recent_trades
                """
            ).fetchone()
        ...
        signal_counts = {str(row["status"]): int(row["count"] or 0) for row in signal_rows}
        blocked_execution = sum(count for status, count in signal_counts.items() if status == "BLOCKED_BY_EXECUTION")
        blocked_risk = sum(count for status, count in signal_counts.items() if status == "BLOCKED_BY_RISK")
        pending_orders = signal_counts.get("PENDING_ORDER", 0)
        expired_orders = signal_counts.get("EXPIRED", 0)
        total_closed = int(trade_row["total_closed_trades"] or 0) if trade_row else 0
        total_fees = float(trade_row["total_fees"] or 0.0) if trade_row else 0.0
        total_funding = float(trade_row["total_funding"] or 0.0) if trade_row else 0.0
        total_friction = float(trade_row["total_friction"] or 0.0) if trade_row else 0.0
        net_pnl = float(trade_row["net_pnl"] or 0.0) if trade_row else 0.0
        gross_estimated_pnl = float(trade_row["gross_estimated_pnl"] or 0.0) if trade_row else 0.0
        return {
            ...
            "gross_estimated_pnl": gross_estimated_pnl,
            "net_pnl": net_pnl,
            "friction_drag_pct": (total_friction / abs(gross_estimated_pnl)) if gross_estimated_pnl else 0.0,
            "avg_fee_per_trade": (total_fees / total_closed) if total_closed else 0.0,
            "partial_exits": int(trade_row["partial_exits"] or 0) if trade_row else 0,
    ...
    def get_state(self, *, limit_trades: int = 200, limit_equity: int = 1200) -> Dict[str, Any]:
        ...
        state["simulation_stats"] = self.get_simulation_stats()
        return state
    ...
    def migrate_trade_simulation_columns(self) -> None:
        """Add paper simulation cost columns to recent_trades if they do not exist."""
        with self._connect() as conn:
            cursor = conn.execute("PRAGMA table_info(recent_trades)")
            columns = {row[1] for row in cursor.fetchall()}
            migrations = {
            ...
            for column, definition in migrations.items():
                if column not in columns:
                    conn.execute(f"ALTER TABLE recent_trades ADD COLUMN {column} {definition}")

    def migrate_signal_forecast_columns(self) -> None:
        """Add forecast tracking columns to signal_journal if they do not exist."""
        with self._connect() as conn:
            cursor = conn.execute("PRAGMA table_info(signal_journal)")
            columns = {row[1] for row in cursor.fetchall()}
            migrations = {
                "forecast_status": "TEXT NOT NULL DEFAULT 'WATCHING'",
                "forecast_result": "TEXT NOT NULL DEFAULT 'watching'",
            ...
            for column, definition in migrations.items():
                if column not in columns:
                    conn.execute(f"ALTER TABLE signal_journal ADD COLUMN {column} {definition}")
